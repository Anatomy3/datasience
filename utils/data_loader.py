...
class DataLoader:
    ...
    def load_dataset(self, filename: str = 'ds_salaries.csv') -> pd.DataFrame:
        ...
        try:
            ...
            print(f"[SUCCESS] Dataset loaded successfully: {len(self._df)} records")
            print(f"[INFO] Columns: {list(self._df.columns)}")
            return self._df
    ...
    def save_processed_data(self, df: pd.DataFrame, filename: str = 'processed_data.pkl'):
        ...
        try:
            ...
            print(f"[SUCCESS] Processed data saved: {file_path}")
        except Exception as e:
